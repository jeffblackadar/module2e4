...
pages=$(curl 'http://search.canadiana.ca/search?q="Shawville"&field=&so=score&df=1919&dt=1939&fmt=json' | jq '.pages')
...
do
        curl 'http://search.canadiana.ca/search/'${i}'?q=montenegr*&field=&so=score&df=1919&dt=1939&fmt=json' | jq '.docs[] | {key}' >> results.txt
done
